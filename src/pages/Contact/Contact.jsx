...
function Contact() {
  ...
  function enviarFormulario(event) {
    ...
    const linkWhatsApp = `https://wa.me/${numeroDoWhatsApp}?text=${encodeURIComponent(
      texto
    )}`;
  ...
  function mascaraTelefone(event) {
    ...
    let telefoneFormatado = textoApenasNumero.replace(
      /^(\d{2})(\d{5})(\d{4})/,
      "($1) $2-$3"
    );
    if (textoApenasNumero.length < 11) {
      telefoneFormatado = textoApenasNumero.replace(
        /^(\d{2})(\d{4})(\d{0,4})/,
        "($1) $2-$3"
      );
    }
